...
class PushSqlE {
    ...
    public function __construct(EntityManager $em, RequestStack $requestStack, ContainerInterface $container, ImageMdE $imagemd) {
    ...
    public function pushCars() {
        $cars = $this->entityManager->getRepository('AppBundle:Cars')->findOneBy(array('id' => $id_add));
        if ($cars) {
            try {
